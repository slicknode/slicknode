      ...
      const interceptor = nock(DUMMY_ENDPOINT)
        .post(() => true, (body) => {
          ...
          if (typeof body === 'string') {
            ...
            if (mockRequest.variables && !content.includes(JSON.stringify(mockRequest.variables))) {
              return false;
            }
            if (mockRequest.operationName && !content.includes(JSON.stringify(mockRequest.operationName))) {
              return false;
            }
            return true;
          }
