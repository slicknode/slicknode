...
describe('pull', () => {
  ...
  test
    ...
    .nock('http://localhost', (loader) =>
      loader
        .get('/repository/core')
        .reply(200, require('./fixtures/modules/core.json'))
    )
    .nock('http://localhost', (loader) =>
      loader
        .get('/repository/auth')
        .reply(200, require('./fixtures/modules/auth.json'))
    )
    .nock('http://localhost', (loader) =>
      loader
        .get('/repository/relay')
        .reply(200, require('./fixtures/modules/relay.json'))
    )

    // Mock source archives for modules
    .nock('http://localhost', (loader) =>
      loader
        .get('/repository/core.zip')
        .replyWithFile(
          200,
          path.join(__dirname, 'fixtures', 'modules', 'core_0.0.1.zip')
        )
    )
    .nock('http://localhost', (loader) =>
      loader
        .get('/repository/auth.zip')
        .replyWithFile(
          200,
          path.join(__dirname, 'fixtures', 'modules', 'auth_0.0.1.zip')
        )
    )
    .nock('http://localhost', (loader) =>
      loader
        .get('/repository/relay.zip')
        .replyWithFile(
          200,
          path.join(__dirname, 'fixtures', 'modules', 'relay_0.0.1.zip')
        )
    )
    ...
    .api(GET_REPOSITORY_URL_QUERY, {
      data: { registryUrl: 'http://localhost/repository/' },
    })
    .workspaceCommand(projectPath('initialized'), ['pull'])
  ...
  test
    ...
    .api(GET_REPOSITORY_URL_QUERY, {
      errors: [{ message: 'Internatl Server Error' }],
      data: null,
    })
    .workspaceCommand(projectPath('initialized'), ['pull'])
  ...
  test
    ...
    // Mock repository detail requests for modules
    .nock('http://localhost', (loader) =>
      loader.get('/repository/auth').reply(404)
    )
    ...
    .workspaceCommand(projectPath('initialized'), ['pull'])
    .catch(
      /Update of module "auth" failed: Metadata could not be loaded. Make sure you are online and try again./
    )
