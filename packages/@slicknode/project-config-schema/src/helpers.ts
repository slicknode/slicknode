...
export function isInputObjectTypeConfig(type: TypeConfig): type is InputObjectTypeConfig {
  return type.kind === TypeKind.INPUT_OBJECT;
}

export function isInterfaceTypeConfig(type: TypeConfig): type is InterfaceTypeConfig {
...
export function isNode(type: TypeConfig): type is ObjectTypeConfig {
  return type.kind === TypeKind.OBJECT && (type.interfaces || []).includes('Node');
}

export function isContent(type: TypeConfig): type is ObjectTypeConfig {
  return type.kind === TypeKind.OBJECT && (type.interfaces || []).includes('Content');
}

export function isContentUnion(type: TypeConfig, typeMap: TypeConfigMap): type is UnionTypeConfig {
...
export function isTypeConfigWithFields(type: TypeConfig): type is (InterfaceTypeConfig | ObjectTypeConfig | InputObjectTypeConfig) {
  return isInterfaceTypeConfig(type) || isObjectTypeConfig(type) || isInputObjectTypeConfig(type);
}
...
export function assertInputObjectTypeConfig(type: TypeConfig): InputObjectTypeConfig {
...
export function assertInterfaceTypeConfig(type: TypeConfig): InterfaceTypeConfig {
